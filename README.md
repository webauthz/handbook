...
The first step of the authorization routine is to request access
to a resource.

If the Webauthz Request URI scheme is `https`, the application redirects the user
...
server. An example of a Webauthz Request URI is
...
The `client_id` was obtained via
the [registration](#registration) step. The `client_state` is an optional value generated by
the application to help it determine what to do with the grant later, if the
grant occurs. The `realm`, `scope`, and `path` parameters MUST be included if they were
...
If the application is a mobile or desktop application, it MAY be
opened directly by the Grant Redirect URI in accordance with its
scheme and authority, or it MAY load a website that causes the application
...
namely in all requests to the same authority and matching path, which
...
Access tokens must be associated to an authority and a path.
...
1. An application
   ...
   with the authority "https://example1.com" and the path "/customer". The same
...
2. The application makes a request to "https://example1.com/directory",
   which has the same authority but a different path that does not match "/profile",
   ...
   and the access token is associated with the authority "https://example1.com" and the
...
3. The application makes a request to "https://example1.com/customer/profile/contacts", and
   it automatically includes the first token because the authority "https://example1.com"
   ...
   the same authority "https://example1.com", the same path "/customer", and now has
...
4. The application makes a request to "https://example2.com/api", and does not include
   any access token because the authority "https://example2.com" does not match any
...
and the new scopes, so that applications only need one token for an authority and path,
...
replaces prior access tokens with new access tokens for the same authority and path.
...
In OAuth 2.0, the authorization server can be separate from the
resource server. This enables an architect to put all the complicated
functionality in one place, and make simpler resource servers that
only need to know how to validate access tokens.

But a lot of complexity can be eliminated by not including the
concept of a separate authorization server in the protocol.

In Webauthz, the application communicates directly with the resource
server to request access. When the user approves the request, the
resource server encodes the `access_token` in whatever manner it
chooses because no other system needs to understand it.

This means the `access_token` can expire, or not expire; it can
be a pointer to a database record or it can be a self-encoded
permission.

For a simple implementation, the "authorization server" and the
"resource server" can be the same server.

Like OAuth 2.0, the Webauthz specification is written with separate
terms so that in more complicated deployments it is clear where to
place server responsibilities.
Although the overview uses the terms "authorization server" and "resource server",
in Webauthz these are referring to the same server.
